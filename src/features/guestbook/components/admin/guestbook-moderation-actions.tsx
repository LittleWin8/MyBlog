...
import { useAdminGuestbook } from "../../hooks/use-guestbook";

interface GuestbookModerationActionsProps {
  entryId: number;
  status: string;
}

export const GuestbookModerationActions = ({
  entryId,
  status,
}: GuestbookModerationActionsProps) => {
  ...
  return (
    <div className="flex items-center justify-end relative" ref={menuRef}>
      <Button
        ...
        title={m.guestbook_action_btn()}
      >
        {isLoading ? (
          <Loader2 size={12} className="animate-spin" />
        ) : (
          <span>[ {m.guestbook_action_btn()} ]</span>
        )}
      </Button>
        ...
        <div className="absolute right-0 top-full mt-2 w-48 bg-background border border-border/30 z-50 p-1 animate-in fade-in zoom-in-95 duration-200">
          <div className="space-y-0.5">
            ...
            {status !== "deleted" && (
              ...
              >
                <span>{m.guestbook_action_trash()}</span>
      ...
      <ConfirmationModal
        ...
        title={m.guestbook_destroy_modal_title()}
        message={m.guestbook_destroy_modal_desc()}
        confirmLabel={m.guestbook_destroy_modal_confirm()}
        isDanger={true}

...
> {
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    ...
    if (!plainText || plainText.trim().length === 0) {
      // Empty entry, leave as pending for manual review
      console.log(
        JSON.stringify({
          message: "guestbook entry is empty, leaving for manual review",
          entryId,
        }),
      );
      return;
    ...
    await step.do("update entry status", async () => {
      ...
      if (moderationResult.safe) {
        await GuestbookRepo.updateEntry(db, entryId, {
          status: "published",
          aiReason: moderationResult.reason,
        });
      }
      // unsafe: keep as "pending" for manual admin review
    });

    // Step 5: Notify admin when entry is flagged for review
    if (!moderationResult.safe) {
    ...
    // Step 6: Send reply notification if entry was approved and is a reply
